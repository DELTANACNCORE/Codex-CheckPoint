...
单独调用 `$synthesize` 不会自行挑选断点，也不会写入项目总结。用户必须明确给出项目名、标签，或明确要求聚类并确认目标范围。选定材料后，脚本才会写入项目总结，并将参与会话和项目总结标记为已知识归档；随后按已记录用户消息数和材料长度判断是否值得提炼长期经验。长会话只提示 Codex 询问用户是否提炼长期经验；短会话明确说明未写入长期经验。无论长度如何，用户可以明确要求强制提炼。
