...
- **可复用经验**：Stop Hook 会把跨项目可迁移的方法、避坑点和验收清单写入 `Claude方案/可复用经验/<项目名>-可复用经验.md`，后续同类项目优先读经验摘要，不恢复长 transcript
- **手动**：`/checkpoint`
...
1. 同项目继续工作时，先读 `Claude方案/<项目名>/项目总结.md`。
2. 同类项目复用经验时，先读 `Claude方案/可复用经验/<项目名>-可复用经验.md` 或相关经验摘要。
3. 只补读 1-2 篇最相关方案/修复记录，避免恢复完整长会话导致 `cache_read_input_tokens` 膨胀。
4. 新项目、新服务器、新报告阶段优先开新会话，用项目总结和经验摘要承接上下文。
